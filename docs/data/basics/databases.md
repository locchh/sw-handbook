...
Where state lives. [Introduction to Data](introduction_to_data.md) covered data types and normalization (1NF→BCNF); this page goes deeper into how a database stores, finds, and protects data. For the full menu of database *types* (document, key-value, wide-column, graph), see [Architecture](../../software/basics/architecture.md) under *Databases* — here we focus on the relational core and when to leave it.
...
An index is a sorted lookup structure (usually a B-tree — a balanced tree, see [Algorithms](../../software/basics/algorithms.md)) that turns an O(n) table scan into an O(log n) lookup.
...
ACID describes guarantees **within one database**. Spread data across machines and you hit the **CAP trade-off** (see [Architecture](../../software/basics/architecture.md)) — many distributed stores relax strict consistency for availability. This looser model is called **BASE**: *Basically Available, Soft state, Eventually consistent*.
