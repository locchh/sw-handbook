...
For installation, see [Machine Setup](machine_setup.md). For command syntax beyond the examples here, use [Linux Commands](linux_commands.md).
...
Replace `main.py` with your application's source path. `make check` checks lint, formatting, and types; `make test` checks behavior. A formatter is not a type checker, and neither replaces tests. Keep the same commands in [CI/CD](cicd.md) so local success means something before a merge.
...
Use [Git](../basics/git.md) for branch and history mechanics, and [Code Review](../basics/code_review.md) for evaluating the resulting change.
...
A local stack is a development environment. Deployment still needs resource sizing, backups, health checks, and a recovery path; see [Running a Server](server_operations.md).
...
Start with Git, an editor, the project's package manager, and its test command. Add formatting and linting for consistent feedback, a task runner when commands repeat, and containers when service dependencies become difficult to reproduce. Introduce AI assistance through the [AI Tools](../../ai/tool_tip/tools.md) guide while keeping the same verification steps.
