...
It assumes a Linux VPS or bare-metal box. For setting up a *development* machine see [Machine Setup](machine_setup.md); for the commands themselves as a flat lookup see [Linux Commands](linux_commands.md).
...
See [Networking & the Web](../basics/networking.md) for the protocol layer beneath this, and [Security](../basics/security.md) for the threat model.
...
2. `df -i` — inodes exhausted? If yes, jump to the small-file hunt in §6.
3. `du -xh / --max-depth=2 | sort -rh | head -20` — where are the bytes?
4. If `du` disagrees with `df`: `lsof +L1`, then restart the holding process.
5. Clear the safe wins: `journalctl --vacuum-size=200M`, `docker system prune -a`, `apt clean`.
6. Fix the cause — add rotation or eviction — then raise the alert threshold to 80%.

### Out of memory

1. `dmesg -T | grep -i 'killed process'` — did the OOM killer fire, and on whom?
2. `ps aux --sort=-%mem | head` — current largest consumers.
3. Watch RSS over time; a steady climb is a leak, a spike is a workload.
4. Short term: add swap or resize. Long term: fix the leak or cap the service with a systemd `MemoryMax=`.

### Load is high, app is slow

1. `uptime` against `nproc` — is the run queue actually oversubscribed?
2. `vmstat 1 5` — is it `r` (CPU), `wa` (disk), `si/so` (swapping), or `st` (steal)?
3. Follow the winner: CPU → `top`/`pidstat`; disk → `iostat -xz`; swap → §5; steal → change host.
4. Only then read application logs.

### Port not reachable

1. `ss -tulpn` on the box — is anything listening, and on which interface?
2. Bound to `127.0.0.1` when it should be `0.0.0.0`? That's the answer more often than not.
3. `ufw status` and the cloud provider's security group — two firewalls, both must allow it.
4. From outside: `curl -sSv` / `mtr` to separate DNS, routing, and application failures.
