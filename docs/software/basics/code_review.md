...
This page owns **reading diffs and reviewing PRs**. For the branch mechanics underneath — merge vs. rebase, conflicts, the daily branch workflow — see [Git: Merge vs Rebase](git.md); for where review sits in the delivery process, see [Lifecycle](lifecycle.md).
...
**Further reading:** [A successful Git branching model](https://nvie.com/posts/a-successful-git-branching-model/) — Vincent Driessen. The branching model this workflow assumes is summarized in [Lifecycle → Version Control](lifecycle.md#version-control).
