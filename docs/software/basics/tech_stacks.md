...
A tech stack is the set of languages, frameworks, databases, and infrastructure used to build a system. This page is a compact reference — for architectural patterns and layering, see [Architecture](architecture.md).
...
See [Architecture § Databases](architecture.md#databases) for the full breakdown. Quick picks:
...
2. **Problem fit.** ML-heavy? Python. High-throughput trading? Rust, C++, or Java. Real-time collab? Node or Elixir.
3. **Ecosystem maturity.** Libraries, hiring pool, Stack Overflow answers, long-term viability.
4. **Operational cost.** Managed services (RDS, Cloud Run) beat self-hosting when you can afford them.
5. **Future-proofing, carefully.** Don't adopt yesterday's fad; don't chase tomorrow's either.
