...
This page owns the **branch mechanics** — fetch vs. pull, `origin/main`, merge, rebase, conflicts, and the daily loop. For reading the resulting diff and reviewing a PR, see [Code Review](code_review.md); for the branching model these commands serve, see [Lifecycle → Version Control](lifecycle.md#version-control); for the commands themselves as a quick lookup, see [Linux Commands → Git Essentials](../tool_tip/linux_commands.md#git-essentials).
