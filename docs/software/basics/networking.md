...
> Load balancers, CDNs, and rate limiting build on these basics — see [Architecture](architecture.md). For the contract layer on top of HTTP, see [APIs](apis.md).
...
*Safe* = no side effects; *idempotent* = repeating it has the same effect as doing it once (this is what makes retries safe — see [APIs](apis.md)). HTTP/2 and /3 add multiplexing so many requests share one connection; status codes live in [APIs](apis.md).
