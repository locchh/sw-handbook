...
For the agents these tools run on top of, see [The LLM Landscape → CLI Coding Agents](../basics/landscape.md#cli-coding-agents); for day-to-day habits when pairing with an AI, see [Tips](../../software/tool_tip/tips.md).
...
3. Add examples, failure cases, and acceptance criteria.
4. Inspect the repository and record relevant constraints.
5. Resolve high-impact questions before planning.
6. Decompose the plan into independently verifiable tasks.
7. Implement one task at a time.
8. Run tests and compare the result with the acceptance criteria.
9. Update the spec if the intended behavior changed.
